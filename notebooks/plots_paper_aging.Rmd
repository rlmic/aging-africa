...
dat = read_dta(
  '../data/preprocessed/policyenvironment.dta'
  )
```
...
dat_0 = dat_0 %>% select(
  govtexpgdp, oopexp, pension_coverage_oldage, hospbeds, physicians, country, cou
  )
...
```{r outcomes_countries, fig.cap="Employment. Age Group and Gender. Countries", echo=FALSE, fig.height=60, fig.width=45, dev = c("png", "tiff", "pdf"), dpi=100}
...
c <- fac_row("work_any", theme_middle()) +
  ylim(0, 1) +
  labs(y = "Employment\n(Proportion)")+ 
  scale_x_discrete(
      breaks=breaks,
...
d <- fac_row("work_hrs", theme_bottom()) + 
  labs(y = "Employment\n(Hours)", x = "Age")+ 
  scale_x_discrete(
      breaks=breaks,
      labels=labels,
...
f <- fac_row("lives_alone", theme_bottom()) + 
  ylim(0, 1) + 
  labs(y = "Living Arrangements\nLives alone\n(Proportion)", x = "Age")+ 
  scale_x_discrete(
      breaks=breaks,
...
g <- fac_row("ill_inj", theme_middle()) + 
  ylim(0, 1) + 
  labs(y = "Health\nIllness & Injury\n(Proportion)")+ 
  scale_x_discrete(
      breaks=breaks,
...
h <- fac_row("ill_inj_stopactivity", theme_middle()) +
...
i <- fac_row("depressed", theme_middle()) + 
  ylim(0, 1) +
...
j <- fac_row("disability", theme_bottom()) + 
  ylim(0, 1) + 
...
ggplot(
  ...
  facet_grid(
    ...
    scales = "free_x", 
    labeller= as_labeller(new_labeller)
    ) +
  ylab("") +
  xlab("") + 
  coord_flip() + 
