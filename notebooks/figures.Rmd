...
fac_row_agg_gen_dis <- function(
  ...
  # Dynamically generate the number of empty strings based on the length of vars
  num_conditions <- length(vars)
  
  # Adjust the labels dynamically for Male and Female based on num_conditions
  f_labels <- data.frame(
    condition = vars,  
    female = c(rep("Male", 1), rep("", num_conditions - 1)),
    label = c("Male", rep("", num_conditions - 1))
  )
  f_labels$condition = factor(f_labels$condition, levels = vars)

  f_labels2 <- data.frame(
    condition = vars,  
    female = c(rep("Female", 1), rep("", num_conditions - 1)),
    label = c("Female", rep("", num_conditions - 1))
  )
  f_labels2$condition = factor(f_labels2$condition, levels = vars)

  f_labels3 <- data.frame(
    condition = vars,  
    female = c(rep("", num_conditions - 1), "Female"),
    label = c(rep("", num_conditions - 1), "Female")
  )
  f_labels3$condition = factor(f_labels3$condition, levels = vars)

  f_labels4 <- data.frame(
    condition = vars,  
    female = c(rep("", num_conditions - 1), "Male"),
    label = c(rep("", num_conditions - 1), "Male")
  )
  f_labels4$condition = factor(f_labels4$condition, levels = vars)
