...
dep_rat_ov60 <- process_dependency_ratio(pop_pro, "depratio_15to59_", countries)
old_age_rat_ov60 <- process_dependency_ratio(pop_pro, "oldagedepratio_15to59_", countries)
dep_rat_ov65 <- process_dependency_ratio(pop_pro, "depratio_15to64_", countries)
old_age_rat_ov65 <- process_dependency_ratio(pop_pro, "oldagedepratio_15to64_", countries)
...
# Define the stacked_labels data frame
...
fac_row_agg_count_out <- function(
  ...
  graph <-data %>%  
    ...
    #geom_line(size = 1) +
    geom_line(size=1.1) +
    geom_line(data = filter(data, (country ==  "Average" & condition!='healthinsurance_ever')), size = 2, linetype="dotdash")+#, linetype="dashed")+
...
fac_row_agg_count_dis <- function(
  ...
  graph <-data %>%  
    ...
    theme_minimal() +
    geom_line(size=1.1) +
    geom_line(data = filter(data, (country ==  "Average" & str_detect(condition, 'diff'))), size = 2, linetype="dotdash")+
...
# General filtering and reshaping into long format for selected variables
to_long_format <- function(df, vars, order_map) {
  df %>%
    select(any_of(vars)) %>%  # Filter for specified numeric columns only
    select(where(is.numeric)) %>%  # Further filter to keep only numeric columns
    pivot_longer(cols = everything(), names_to = "variable", values_to = "value") %>%
    mutate(order = case_when(
      country %in% names(order_map) ~ order_map[country],
      TRUE ~ NA_integer_
    )) %>%
...
# Define ordering for countries and variables to pivot
country_order <- c(
  'South Africa' = 1, 'Ghana' = 2, 'Nigeria' = 3,
  'Tanzania' = 4, 'Ethiopia' = 5, 'Uganda' = 6,
  'Malawi' = 7, 'Niger' = 8
)

# Apply initial transformations on `dat`
dat <- dat %>%
  arrange(desc(govtexpgdp)) %>%
  mutate(
    cou = ifelse(country %in% countries, 1, 0),
    oopexp = 100 - oopexp
  )

# Apply the transformation and reshaping
dat_long <- to_long_format(dat, head(var_sum, -1), country_order)

# Additional transformations for income groups
income_group_filter <- function(df, pattern) {
  df %>%
    filter(str_detect(country, pattern)) %>%
    select(-cou) %>%
    pivot_longer(cols = physicians:uhcidx, names_to = "variable", values_to = "value")
}

# Filter data based on income groups
high_inco <- income_group_filter(dat, "HIC")
low_middle_inco <- income_group_filter(dat, "LMICs_nonSSA")
sub_sahar_inco <- income_group_filter(dat, "Sub-Saharan Africa")

# Remaining dataset transformations
dat <- dat %>%
  arrange(desc(govtexpgdp)) %>%
  mutate(
    cou = ifelse(country %in% countries, 1, 0),
...
# Split data based on cou and reshape each subset
dat_1 <- to_long_format(filter(dat, cou != 0), var_sum, country_order)
dat_0 <- to_long_format(filter(dat, cou == 0), var_sum, NULL)
...
depratov60 <- ggplot(
  ...
  geom_line(
    aes(color = countryregion, size=countryregion, linetype = countryregion)
    ) + 
...
```{r ov60_dep_rat, eval=FALSE, fig.height=7.5, fig.width=18, dev=c("png"), dpi=100, include=FALSE}
# Figure 2: Projected Population Growth and Dependency Ratios
# Panels E & F
...
```{r out_agg_ctr_wgt, fig.cap="Employment. Age Group and Gender.", echo=FALSE,  fig.height=25, fig.width=25, dev = c("png"), dpi=100}
...
```{r}
out_dis_weigted_gender = trans_input_count_gender(
...
fac_row_agg_count_out_gender_ <- function(
    data,
    vars,
    labels,
    forma_theme,
...
```{r out_agg_gen_wgt, fig.cap="Employment. Age Group and Gender.", echo=FALSE,  fig.height=25, fig.width=25, dev = c("png"), dpi=100}

# Figure 4: Employment, Health, & Living Arrangements over the Life Course: by Gender
fac_row_agg_count_out_gender_(
  out_dis_weigted_gender,
  measures,
  measures_labs_agg,
  theme_top_count()
  )+ 
...
```{r ov65_dep_rat, eval=FALSE, fig.height=7.5, fig.width=18, dev=c("png"), dpi=100, include=FALSE}
