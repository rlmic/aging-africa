...
projections = read_dta(
  '../data/preprocessed/population_projections_t1.dta'
  )
...
subsaharan_nonweighted = read_dta(
  "../data/preprocessed/allcountries_means5year_noweights_all.dta"
)
...
countries_nonweighted = read_dta(
  "../data/preprocessed/allcountries_means5year_noweights_countries.dta"
)
pop_pro = read_dta(
  "../data/preprocessed/population_projections_allcountries.dta"
)
...
oldageratov65 = ggplot(old_age_rat_ov65, aes(year, ratio, group=countryregion)) +
  ...
  labs(x="", y="", title='**B.-** Over 60 Population (Dependency Ratio)')+
    theme_minimal() +
...
PRUEBATOT = ggplot(
  ...
  geom_line(
    aes(color = countryregion, size=countryregion)#, size =if_else(countryregion=='Sub-Saharan Africa', 0.91, 0.9))
    ) + 
   scale_size_manual(values = c(1.9,0.9, 0.9)) +
    geom_label(data = . %>%  mutate(condition =  case_when(
    condition %in% c(1990) ~ NA,
    condition == 2020 ~ 2005,
    condition == 2050 ~ 2035,
    condition ==2100 ~ 2075),
    measurement = case_when(name_lab=='+ 63 %' ~ 2776948, 
                            name_lab=='- 8 %' ~ 1600170,
                            name_lab=='- 42 %' ~ 1039655,
                            ...
                            name_lab=='- 8 %' ~ 1068783,
                            name_lab=='+ 90 %' ~1753968)),
    aes(label = name_lab, colour=countryregion),fill='white', 
    #nudge_x =-15,
    #nudge_y = 0.25,
    size=5,
    check_overlap = FALSE
  ...
  scale_x_continuous(
    ...
    geom_brace(aes(x=c(1990,2019), y=c(-5000,40000)), inherit.data=F, rotate = 180, color="grey") +
  geom_brace(aes(x=c(2021,2049), y=c(-5000,40000)), inherit.data=F, rotate = 180, color="grey") +
    geom_brace(aes(x=c(2051,2100), y=c(-5000,40000)), inherit.data=F, rotate = 180, color="grey")
...
PRUEBA= ggplot(
  ...
  geom_line(
   ...
   scale_size_manual(values = c(1.9,0.9, 0.9)) +
    ...
    condition == 2050 ~ 2035,
    condition ==2100 ~ 2075),
    measurement = case_when(name_lab=='+ 313 %' ~ 400078.6, 
  ...
  scale_x_continuous(
    ...
    geom_brace(aes(x=c(1990,2019), y=c(-5000,4000)), inherit.data=F, rotate = 180, color="grey") +
  geom_brace(aes(x=c(2021,2049), y=c(-5000,4000)), inherit.data=F, rotate = 180, color="grey") +
    geom_brace(aes(x=c(2051,2100), y=c(-5000,4000)), inherit.data=F, rotate = 180, color="grey")+
    scale_color_manual(values=pal)+   
    scale_y_continuous(
...
labeller_health <- c(
  'oopexp'="**A.-** 100 - Out of Pocket<br>Health Expenditure<br>(Percentage)<br>",
  `govtexpgdp`="**B.-** Gov. Health<br>Expenditure<br>(Percentage of GDP)<br>",
...
guides(
 ...
 # scale_color_manual("Regional\nAverages", values = colo) +
     ...
     scale_linetype_manual(name = "Regional\nAverages", values = c(1, 2, 1), 
                      guide = guide_legend(title="Regional\nAverages", reverse=TRUE, override.aes = list(color = c("#8eb67d", "#686868", "black"))))+
                      
  theme_minimal() +

  theme_spac() +
  geom_text(label = rep(c("South Africa", "Malawi", "Niger", "Tanzania", "Ghana", "Ethiopia", "Uganda", "Nigeria"),4), hjust = ifelse(counn== 'South Africa', 1, -0.05), color="#344771", size=5)
  return (graph)
...
```{r ov60_pop, echo=FALSE, fig.height=7.5, fig.width=18, dev = c("png"), dpi=300}
# Figure 1 (A & B)
fig1a | fig1b  
```

```{r ov60_growth, echo=FALSE, fig.height=7.5, fig.width=18, dev = c("png"), dpi=300}
# Figure 1 (C & D)
PRUEBATOT| PRUEBA
```

```{r ov60_dep_rat, echo=FALSE, fig.height=7.5, fig.width=18, dev = c("png"), dpi=300}
# Figure 1 (E & F)
...
# Figure 2: Key Indicators across Sub-Saharan African Countries
main_outco(
  vars=var_env,
...
# Figure 3: Over-60 Population Growth
newggslopegraph(
  dataframe = proj_pop,
  Times = measurement,
  Measurement = value,
...
# Figure 8: Health and Health Care
main_outco(
  vars=var_hea,
...
```{r ov65_dep_rat, echo=FALSE, fig.height=7.5, fig.width=18, dev = c("png"), dpi=300}
...
```{r outcomes_aggregate_nonweighted, fig.cap="Employment. Age Group and Gender.", echo=FALSE,  fig.height=25, fig.width=25, dev = c("png"), dpi=200}
...
```{r disabilities_aggregate_nonweighted, echo=FALSE,  fig.height=12, fig.width=19, dev = c("png"), dpi=200}
