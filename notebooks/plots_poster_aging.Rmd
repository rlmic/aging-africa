...
a <- fac_row("urban", theme_top()) + labs(y = "Urban Status\n(Proportion)")
b <- fac_row("work_hrs", theme_middle()) + labs(y = "Employment\n(Hours)")
c <- fac_row("work_any", theme_middle_wline()) + labs(y = "Employment\n(Proportion)")
d <- fac_row("lives_alone", theme_middle_wline()) + ylim(0, 0.6) + labs(y = "Living Arrangements\nLives alone?\n(Proportion)")
e <- fac_row("ill_inj", theme_middle()) + ylim(0, 0.6) + labs(y = "Health\nIllness or Injury?\n(Proportion)")
f <- fac_row("ill_inj_stopactivity", theme_middle()) + ylim(0, 0.6) + labs(y = "Health\nStopped Activities\n(Proportion)")
g <- fac_row("depressed", theme_bottom()) + ylim(0, 0.6) + labs(y = "Health\nDepression", x = "Age", caption = paste(str_wrap("*Measurements derive from the Living Standards Measurement Study (LSMS), except for the outcomes corresponding to South Africa and Ghana, which come from the National Income Dynamics Study (NIDS) and the Ghana Socioeconomic Panel Survey (GSPS, respectively.", 170), collapse = "\n" ), adj=+1)
plot_grid(plotlist=list(a, b, c, d, e, f, g), ncol=1, align='v', rel_heights = c(10, 5, 5, 5, 5, 5, 7))
