...
fac_row_popu <- function(
  ...
  graph <- ssa %>%
    ...
    group_by(country, age_group5, gender, region) %>%
    tally() %>% 
    ggplot(
      aes(x = age_group5, y = n, colour = gender, group=gender)
      ) + 
    theme_minimal() +
    geom_line(size = 1.3, aes(linetype=gender)) +
...
```{r employmenthours, fig.cap="Employment. Age Group and Gender.", echo=FALSE, fig.height=35, fig.width=20, dev = c("png", "tiff"), dpi=400}
a <- fac_row_popu( theme_top()) + labs(y = "Population\n(Count)")
b <- fac_row("urban", theme_middle_wline()) + ylim(0, 1) + labs(y = "Urban Status\n(Proportion)")
c <- fac_row("work_any", theme_middle()) + ylim(0, 1) + labs(y = "Employment\n(Proportion)")
d <- fac_row("work_hrs", theme_middle_wline()) + labs(y = "Employment\n(Hours)")
e <- fac_row("notmarried", theme_middle()) + ylim(0, 1) + labs(y = "Living Arrangements\nUnmarried\n(Proportion)")
f <- fac_row("lives_alone", theme_middle_wline()) + ylim(0, 1) + labs(y = "Living Arrangements\nLives alone\n(Proportion)")
...
plot_grid(plotlist=list(a, b, c, d, e, f, g, h, i), ncol=1, align='v', rel_heights = c(7, 5, 5, 5, 5, 5, 5, 5, 13))
...
proj_pop = proj_pop %>% 
  filter(
    countryregion!='World' & countryregion!='Sub-Saharan Africa'
    )

proj_grow = 
  projections %>% 
  rename(
  '1990-2020'= growth_pop_60plus_1990to2020, 
  '2020-2050'= growth_pop_60plus_2020to2050
  ) %>% select(
  countryregion, '1990-2020', '2020-2050'
)
...
```{r popplus60, echo=FALSE, fig.height=18, fig.width=10, dev = c("png", "tiff"), dpi=600}
newggslopegraph(
  dataframe = proj_pop,
  Times = measurement,
  Measurement = value,
  ...
  SubTitleTextSize = 25,
  LineThickness = 1,
  LineColor = c("#8eb67d", rep("#000C66",1), "#8eb67d", "#000C66", "#808080", rep("#8eb67d",2)),
  WiderLabels = TRUE,
  ThemeChoice = "minimal"
  )
```

```{r growplus60, echo=FALSE, fig.height=18, fig.width=10, dev = c("png", "tiff"), dpi=600}
newggslopegraph(
  dataframe = proj_grow,
  Times = measurement,
  Measurement = value,
  ...
  TitleTextSize = 35,
  LineThickness = 1,
  LineColor = c(rep("#8eb67d",5), "#2E8B57", rep("#8eb67d",2), "#000C66", rep("#8eb67d",1)),
  WiderLabels = TRUE,
  ThemeChoice = "minimal"
  )
```
