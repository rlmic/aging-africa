...
gender_weighted <- load_data("allcountries_means5year_weightspopsurvey_countrygender.dta")
...
fac_row_agg_count_out_gender_ <- function(
  ...
  f_labels3$condition = factor(f_labels3$condition, levels = condition_order)
  ...
  f_labels4$condition = factor(f_labels4$condition, levels = condition_order)
  ...
  graph <-data %>%  
    filter(condition!='healthinsurance_ever') %>% 
    ggplot(
      aes(x = age_group5, y = measurement, group = female, color = female, linetype = female) # Added linetype aesthetic
      ) +
    facet_wrap(
      . ~condition, 
      ncol = 3, 
...
```{r out_agg_ctr_wgt_gen, fig.cap="Employment. Age Group and Gender.", echo=FALSE,  fig.height=25, fig.width=25, dev = c("png"), dpi=100}
...
fac_row_agg_gen_dis <- function(
  ...
  graph <- data %>%  
    ...
    geom_text(x = "72", y = 0.15, aes(label = label), data = f_labels3, size = 9, color = "#050A30") +
    geom_text(x = "75", y = 0.17, aes(label = label), data = f_labels4, size = 9, color = "#8eb67d") +
...
fac_row_agg_gen_dis(
  ...
  scale_y_continuous(breaks=c(0,0.5, 1), labels = c('0', '–', '1'), limits = c(0, 1)) +   theme(legend.position = "none")  # Remove the legend
